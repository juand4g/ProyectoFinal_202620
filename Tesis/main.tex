...
El cartografiado \emph{VISTA Variables in the V\'ia L\'actea} (VVV)\footnote{P\'agina web del proyecto: \url{https://vvvsurvey.org/}} es un relevamiento p\'ublico del Observatorio Europeo Austral. Observa el bulbo de la V\'ia L\'actea y una secci\'on adyacente de su disco con el instrumento VIRCAM del telescopio VISTA de 4~m, en cinco bandas del infrarrojo cercano ($Z=0.87$, $Y=1.02$, $J=1.25$, $H=1.65$ y $K_s=2.14\ \mu$m) \citep{vvv_survey,Fajardo2026}. La regi\'on original de $562$ grados cuadrados se cubri\'o con una cadencia de aproximadamente 70 \'epocas por campo a lo largo de una l\'inea base de seis a\~nos (2010--2016), acumulando fotometr\'ia para m\'as de 288 millones de fuentes con una precisi\'on mejor a 0.05 magnitudes para estrellas con $K_s\lesssim16$ \citep{Fajardo2026}. Su extensi\'on, VVVX, ampli\'o la cobertura espacial original de $562$ a $1700$ grados cuadrados y a\~nadi\'o \'epocas adicionales de observaci\'on entre 2016 y 2023. Entre ambos cartografiados se acumularon aproximadamente $4200$ horas de observaci\'on, con las que se produjo un cat\'alogo de fuentes variables en banda $K_s$ del orden de $10^7$ objetos \citep{saito_2024}. La cobertura en el infrarrojo cercano hace de VVV/VVVX una fuente de datos complementaria a OGLE: la extinci\'on interestelar en banda $K_s$ es aproximadamente una d\'ecima parte de la extinci\'on en banda $V$ ($A_{K_s}/A_V\sim0.1$), lo cual permite penetrar regiones de alta extinci\'on interestelar donde la fotometr\'ia \'optica se ve fuertemente afectada \citep{Fajardo2026}. En contraste con OGLE, la cadencia de observaci\'on de VVV/VVVX es t\'ipicamente m\'as dispersa e irregular, con decenas de \'epocas repartidas a lo largo de varios a\~nos en lugar de observaciones casi diarias. Esto la convierte en un caso de prueba exigente para la determinaci\'on de periodicidad bajo muestreo irregular. La Figura~\ref{fig:vista-telescopio} muestra el telescopio VISTA en su recinto del Observatorio Paranal.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{img/vista-gerd-hd.png}
	\caption[Telescopio VISTA del Observatorio Paranal]{El telescopio VISTA de 4~m, en su recinto del Observatorio Paranal, con el Very Large Telescope (VLT) visible al fondo sobre el cerro vecino. El instrumento VIRCAM, montado en el foco Cassegrain, es el que provee la fotometr\'ia infrarroja de VVV/VVVX. Cr\'edito: G. H\"udepohl/ESO, tomada de la p\'agina del proyecto VVV \citep{vvv_survey}.}
	\label{fig:vista-telescopio}
\end{figure}

\subsection{TESS}

El \emph{Transiting Exoplanet Survey Satellite} (TESS)\footnote{P\'agina web del proyecto: \url{https://tess.mit.edu/}} es una misi\'on espacial de la NASA de la clase Explorer, dise\~nada originalmente para la detecci\'on de exoplanetas en tr\'ansito alrededor de estrellas brillantes y cercanas \citep{ricker_2015}. La Figura~\ref{fig:tess-satelite} muestra una concepci\'on art\'istica del sat\'elite en \'orbita.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{img/tessinspacerender16by9.jpg}
	\caption[Concepci\'on art\'istica del sat\'elite TESS]{Concepci\'on art\'istica del sat\'elite TESS en \'orbita, mostrando las cuatro c\'amaras CCD, la antena de alta ganancia y los dos paneles solares desplegables. Cr\'edito: NASA.}
	\label{fig:tess-satelite}
\end{figure}

TESS emplea cuatro c\'amaras CCD id\'enticas de campo amplio, cada una con un objetivo \'optico de siete elementos con un di\'ametro de apertura de entrada de 10.5~cm y una raz\'on focal $f/1.4$, sensible en un ancho de banda \'optico de 600 a 1000~nm \citep{ricker_2015}. Cada c\'amara cubre un campo individual de $24^\circ\times24^\circ$, y las cuatro en conjunto forman un campo de visi\'on combinado de $24^\circ\times96^\circ$ (2300~grados cuadrados) \citep{ricker_2015}. El detector de cada c\'amara es un mosaico de cuatro CCD de $2048\times2048$ p\'ixeles, con un tama\~no f\'isico de p\'ixel de 15~$\mu$m, equivalente a unos 21 segundos de arco proyectados en el cielo \citep{ricker_2015}. Esta resoluci\'on espacial es notablemente menor que la de OGLE o VVV, lo cual puede producir contaminaci\'on por estrellas vecinas en campos densamente poblados y constituye una limitaci\'on adicional a considerar al aplicar el m\'etodo combinado sobre datos de TESS \citep{Elizabethson2023}. El sat\'elite utiliza el bus LEOStar-2/750, con dos paneles solares desplegables (potencia total generada de 415~W frente a un consumo estimado de 290~W) y una antena de alta ganancia de banda Ka para la transmisi\'on de datos \citep{ricker_2015}. Se ubica en una \'orbita altamente el\'iptica de 13.7~d\'ias alrededor de la Tierra, con perigeo y apogeo nominales de 17 y 59 radios terrestres, respectivamente, en resonancia 2:1 con la \'orbita lunar \citep{ricker_2015}. Monitorea al menos 200\,000 estrellas de secuencia principal con cadencias de dos minutos para objetivos preseleccionados, adem\'as de im\'agenes de campo completo cada treinta minutos \citep{ricker_2015}. Su dise\~no est\'a optimizado para la detecci\'on de tr\'ansitos planetarios, pero la combinaci\'on de alta cadencia y l\'ineas base fotom\'etricas continuas de semanas a meses ha hecho de TESS una fuente cada vez m\'as explotada para la determinaci\'on general de periodos de estrellas variables. \citet{kepler_2026} lo ilustran recientemente con la determinaci\'on de periodos fotom\'etricos coherentes para variables catacl\'ismicas. Tras varios a\~nos de operaci\'on, TESS ha cubierto aproximadamente el 85\% del cielo con al menos 27 d\'ias de observaci\'on continua por campo \citep{Elizabethson2023}. Para este trabajo, TESS aporta el caso de muestreo m\'as denso y regular entre las tres fuentes de datos reales consideradas, y sirve como contraste frente a la irregularidad caracter\'istica de OGLE y VVV/VVVX.
...
\chapter{Resultados de la caracterizaci\'on individual}
\label{ch:resultados}

Antes de dise\~nar el esquema de integraci\'on descrito en la Secci\'on~\ref{sec:metodo-combinado}, es necesario caracterizar por separado el desempe\~no del periodograma de Lomb--Scargle y del m\'etodo de m\'inima entrop\'ia sobre datos reales, siguiendo el primer objetivo espec\'ifico de este trabajo (Secci\'on~\ref{sec:objetivos-especificos}). Este cap\'itulo presenta dicha caracterizaci\'on para ambos m\'etodos aplicados de forma individual, es decir, antes de cualquier combinaci\'on entre ellos.

Para ambos m\'etodos se utiliz\'o la misma muestra aleatoria del OCVS descrita en la Secci\'on~\ref{sec:datos-reales} (semilla aleatoria fija, de modo que ambos m\'etodos se eval\'uan exactamente sobre las mismas estrellas): 500 binarias eclipsantes, 500 Cefeidas cl\'asicas y 500 RR Lyrae, cada una con su fotometr\'ia individual en banda $I$ de OGLE. La mediana del n\'umero de puntos observacionales por curva de luz es de 579 para las binarias y de aproximadamente 470--490 para Cefeidas y RR Lyrae, sobre l\'ineas base t\'ipicas de entre 4 y 6 a\~nos. Los rangos de b\'usqueda de periodo de prueba fueron, para ambos m\'etodos, de 0.2 a 250~d para Cefeidas y de 0.1 a 1.2~d para RR Lyrae; para binarias, el periodograma de Lomb--Scargle se evalu\'o en el rango completo de 0.1 a 3000~d, mientras que el m\'etodo de m\'inima entrop\'ia se acot\'o a 0.1--100~d, dado que con el rango completo converge sistem\'aticamente a un alias estacional de $\sim$350--370~d en vez del periodo orbital real. El desempe\~no de cada m\'etodo se cuantific\'o mediante el error relativo del periodo recuperado, $|P_{\text{encontrado}} - P_{\text{cat}}|/P_{\text{cat}}$, y mediante la tasa de aciertos, definida como la fracci\'on de estrellas con error relativo menor al 1\%. Dado que ambos m\'etodos pueden confundir el periodo real con un arm\'onico simple de este \citep{Graham2013}, se reporta tambi\'en el error relativo tras una correcci\'on de alias que escoge, de entre $P$, $P/2$, $2P$, $P/3$ y $3P$, el candidato m\'as cercano al periodo catalogado.

\section{Caracterizaci\'on del periodograma de Lomb--Scargle}
\label{sec:resultados-ls}

La Figura~\ref{fig:ls-resumen} resume el desempe\~no del periodograma de Lomb--Scargle sobre las 1500 estrellas de la muestra, con el error relativo (tras correcci\'on de alias) por tipo de estrella a la izquierda, y la frecuencia recuperada frente a la frecuencia catalogada a la derecha.

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{img/ls_resumen_error_relativo.png}
	\caption[Exactitud y precisi\'on del periodograma de Lomb--Scargle, por tipo de estrella]{Izquierda: distribuci\'on del error relativo del periodo recuperado por el periodograma de Lomb--Scargle (con correcci\'on de alias), por tipo de estrella. Derecha: frecuencia recuperada vs. frecuencia catalogada, coloreada por tipo de estrella (naranja: binarias; azul: Cefeidas cl\'asicas; verde: RR Lyrae), con la identidad $f_{LS}=f_{\text{cat}}$ en negro discontinuo y, en gris y cada vez m\'as tenues, las diagonales correspondientes a los m\'ultiplos del periodo de $\times2$ a $\times6$ (es decir, $P/n$ y $nP$ para $n=2,\dots,6$). Elaboraci\'on propia sobre 500 binarias, 500 Cefeidas cl\'asicas y 500 RR Lyrae del OCVS.}
	\label{fig:ls-resumen}
\end{figure}

La Tabla~\ref{tab:resultados-ls} resume los estad\'isticos num\'ericos correspondientes a esta figura.

\begin{table}[H]
	\centering
	\begin{tabular}{ccccc}
		\hline
		\textbf{Tipo} & \textbf{Mediana error} & \textbf{Aciertos (1\%)} & \textbf{Aciertos con alias (1\%)} \\
		\hline
		Binaria & 0.5000 & 3.8\% & 78.0\% \\
		Cefeida cl\'asica & 0.00004 & 98.2\% & 98.2\% \\
		RR Lyrae & 0.00001 & 98.4\% & 98.6\% \\
		\hline
	\end{tabular}
	\caption[Estad\'isticos del periodograma de Lomb--Scargle, por tipo de estrella]{Mediana del error relativo del periodo (con correcci\'on de alias) y tasa de aciertos, con y sin dicha correcci\'on, para el periodograma de Lomb--Scargle sobre la muestra de 1500 estrellas del OCVS.}
	\label{tab:resultados-ls}
\end{table}

El resultado m\'as claro es la fortaleza de Lomb--Scargle sobre morfolog\'ias cercanas a una sinusoide: tanto para Cefeidas cl\'asicas como para RR Lyrae, la mediana del error relativo es del orden de $10^{-5}$--$10^{-4}$ y la tasa de aciertos supera el 98\%, sin necesidad siquiera de correcci\'on de alias. La Figura~\ref{fig:ls-cefeida-ejemplo} ilustra un caso t\'ipico.

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{img/ls_periodograma_cefeida_ejemplo.png}
	\caption[Frecuenciograma de Lomb--Scargle de una Cefeida cl\'asica]{Arriba: frecuenciograma de Lomb--Scargle de la Cefeida cl\'asica OGLE-LMC-CEP-0173, con la frecuencia catalogada y la de m\'axima potencia marcadas con parejas de tri\'angulos en el borde del recuadro. Abajo, izquierda: curva de luz real, plegada con el periodo catalogado. Abajo, derecha: curva de luz encontrada, plegada con el periodo de Lomb--Scargle. Ambas curvas de luz se muestran en magnitud vs. fase, con dos ciclos, en paneles separados para facilitar la comparaci\'on. El acuerdo entre ambas es casi perfecto.}
	\label{fig:ls-cefeida-ejemplo}
\end{figure}

El panorama es radicalmente distinto para las binarias eclipsantes: la mediana del error relativo sin corregir es de exactamente 0.5, y solo el 3.8\% de las binarias se recuperan dentro del 1\% de tolerancia. Esto no es ruido ni un fallo aleatorio del m\'etodo, sino un patr\'on sistem\'atico y f\'isicamente interpretable, visible en el panel derecho de la Figura~\ref{fig:ls-resumen}: la inmensa mayor\'ia de las binarias se ubican sobre la diagonal de m\'ultiplo $\times2$, es decir, Lomb--Scargle recupera consistentemente la \emph{mitad} del periodo orbital real. La raz\'on f\'isica es directa: en un sistema eclipsante t\'ipico de tipo Algol, con dos m\'inimos de profundidad similar por periodo orbital, la curva de luz se parece m\'as a una sinusoide de \emph{doble} frecuencia que a una de la frecuencia fundamental, por lo que el ajuste sinusoidal de m\'inimos cuadrados subyacente al periodograma concentra su potencia en el arm\'onico $2f_{\text{orbital}}$ en lugar de en $f_{\text{orbital}}$. Al aplicar la correcci\'on de alias, la tasa de aciertos de las binarias sube de 3.8\% a 78.0\%, confirmando que se trata precisamente de este problema de arm\'onico dominante y no de un fallo m\'as profundo del m\'etodo. La Figura~\ref{fig:ls-binaria-alias} muestra un caso representativo.

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{img/ls_periodograma_binaria_alias.png}
	\caption[Frecuenciograma de Lomb--Scargle de una binaria eclipsante, con alias en $P/2$]{Arriba: frecuenciograma de Lomb--Scargle de la binaria eclipsante OGLE-LMC-ECL-01807, mostrando c\'omo el pico de mayor potencia se ubica al doble de la frecuencia catalogada. Abajo, izquierda: curva de luz real, plegada con el periodo catalogado, con los dos eclipses de profundidad distinta separados por medio periodo. Abajo, derecha: curva de luz encontrada, plegada con el periodo de Lomb--Scargle (mitad del real), en la que ambos eclipses se superponen dentro de un \'unico ciclo aparente.}
	\label{fig:ls-binaria-alias}
\end{figure}

No todas las binarias con error grande se explican por este alias limpio de $P/2$: un subconjunto minoritario, t\'ipicamente aquel con periodos orbitales m\'as largos relativos a su l\'inea base observacional o con eclipses particularmente breves, no mejora sustancialmente ni siquiera tras la correcci\'on de alias, lo que sugiere que para esas estrellas el periodograma converge directamente a una frecuencia esp\'urea no relacionada de forma simple con el periodo orbital real.

\section{Caracterizaci\'on del m\'etodo de m\'inima entrop\'ia}
\label{sec:resultados-entropia}

La Figura~\ref{fig:entropia-resumen} presenta el an\'alogo, para el m\'etodo de m\'inima entrop\'ia, de la Figura~\ref{fig:ls-resumen}.

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{img/entropia_resumen_error_relativo.png}
	\caption[Exactitud y precisi\'on del m\'etodo de m\'inima entrop\'ia, por tipo de estrella]{Izquierda: distribuci\'on del error relativo del periodo recuperado por el m\'etodo de m\'inima entrop\'ia (con correcci\'on de alias), por tipo de estrella. Derecha: frecuencia recuperada vs. frecuencia catalogada, con el mismo esquema de colores y l\'ineas que la Figura~\ref{fig:ls-resumen} (identidad $f_{\text{entrop\'ia}}=f_{\text{cat}}$ en negro discontinuo, m\'ultiplos del periodo de $\times2$ a $\times6$ en gris). Misma muestra que la Figura~\ref{fig:ls-resumen}.}
	\label{fig:entropia-resumen}
\end{figure}

La Tabla~\ref{tab:resultados-entropia} resume los estad\'isticos correspondientes.
...
\begin{table}[H]
	...
	\begin{tabular}{ccccc}
		...
		\textbf{Tipo} & \textbf{Mediana error} & \textbf{Aciertos (1\%)} & \textbf{Aciertos con alias (1\%)} \\
		\hline
		Binaria & 0.9688 & 14.6\% & 32.0\% \\
		Cefeida cl\'asica & 1.0009 & 20.6\% & 45.2\% \\
		RR Lyrae & 0.00007 & 63.0\% & 80.0\% \\
		\hline
	\end{tabular}
	\caption[Estad\'isticos del m\'etodo de m\'inima entrop\'ia, por tipo de estrella]{Mediana del error relativo del periodo (con correcci\'on de alias) y tasa de aciertos, con y sin dicha correcci\'on, para el m\'etodo de m\'inima entrop\'ia sobre la misma muestra de 1500 estrellas.}
	\label{tab:resultados-entropia}
\end{table}

El contraste con la Tabla~\ref{tab:resultados-ls} es notable: aplicado de forma individual y sin ning\'un ajuste adicional, el m\'etodo de m\'inima entrop\'ia obtiene un desempe\~no considerablemente peor que Lomb--Scargle en los tres tipos de estrella, incluyendo las Cefeidas y RR Lyrae en las que Lomb--Scargle es casi perfecto. Esto contradice, a primera vista, la expectativa te\'orica de la Secci\'on~\ref{sec:estado-del-arte} seg\'un la cual el criterio de entrop\'ia, al no asumir ninguna forma funcional, deber\'ia ser al menos tan robusto como Lomb--Scargle frente a morfolog\'ias sencillas.

El caso de las RR Lyrae, el m\'as favorable de los tres para este m\'etodo, permite entender por qu\'e. Con una tasa de aciertos del 63\% sin corregir (80\% con correcci\'on de alias), el m\'etodo funciona razonablemente bien, como ilustra la Figura~\ref{fig:entropia-rrlyrae-ejemplo}, pero est\'a lejos del $\sim$98\% de Lomb--Scargle sobre el mismo tipo de estrella.

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{img/entropia_periodograma_rrlyrae_ejemplo.png}
	\caption[Frecuenciograma de m\'inima entrop\'ia de una RR Lyrae]{Arriba: frecuenciograma de entrop\'ia de Shannon de la RR Lyrae OGLE-LMC-RRLYR-16164; a diferencia de las gr\'aficas de versiones anteriores de este trabajo, el eje vertical no se invierte, de modo que el m\'inimo de entrop\'ia (mejor periodo) aparece como un m\'inimo real de la curva. Abajo, izquierda: curva de luz real, plegada con el periodo catalogado. Abajo, derecha: curva de luz encontrada, plegada con el periodo de m\'inima entrop\'ia. Ambas curvas de luz se muestran en magnitud vs. fase, con dos ciclos, en paneles separados.}
	\label{fig:entropia-rrlyrae-ejemplo}
\end{figure}

Al examinar la distribuci\'on de la raz\'on $P_{\text{entrop\'ia}}/P_{\text{cat}}$ para las RR Lyrae, se encuentra que un 17.2\% adicional de los casos converge, no a un valor arbitrario, sino limpiamente al \emph{doble} del periodo catalogado. Esto es consistente con una limitaci\'on conocida de los m\'etodos de orden de fase frente a curvas de luz de una sola joroba por ciclo, como las de las RR Lyrae: al plegar la curva con el doble del periodo real, cada ciclo verdadero se reparte en dos mitades de fase que, por la simetr\'ia aproximada de la curva, se superponen de forma casi tan ordenada como con el periodo correcto, produciendo una entrop\'ia casi igual de baja y, por tanto, una degeneraci\'on que el criterio de m\'inima entrop\'ia por s\'i solo no puede resolver.

El desempe\~no es sustancialmente peor a\'un para Cefeidas cl\'asicas y binarias eclipsantes, con medianas de error relativo pr\'acticamente iguales a 1 (es decir, t\'ipicamente equivocado por un factor cercano a dos) y tasas de acierto, incluso tras correcci\'on de alias, de apenas 45.2\% y 32.0\% respectivamente. Para las Cefeidas, adem\'as del mismo problema de degeneraci\'on en $2P$ que en las RR Lyrae (presente en un 18.0\% de los casos), se observa una cola de fallos catastr\'oficos en los que el periodo recuperado no guarda ninguna relaci\'on simple con el periodo real, alcanzando errores relativos de varios \'ordenes de magnitud. La Figura~\ref{fig:entropia-cefeida-falla} muestra un ejemplo extremo.

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{img/entropia_periodograma_cefeida_falla.png}
	\caption[Falla catastr\'ofica del m\'etodo de m\'inima entrop\'ia en una Cefeida]{Arriba: frecuenciograma de entrop\'ia de la Cefeida cl\'asica OGLE-GD-CEP-1856 ($P_{\text{cat}}=0.814$~d). El m\'inimo global de entrop\'ia se ubica en $P\approx222$~d, muy por fuera de cualquier m\'ultiplo simple del periodo real. Abajo, izquierda: curva de luz real, plegada con el periodo catalogado, mostrando la variaci\'on peri\'odica gen\'uina de la estrella. Abajo, derecha: curva de luz encontrada, plegada con el periodo esp\'ureo de m\'inima entrop\'ia, indistinguible de ruido.}
	\label{fig:entropia-cefeida-falla}
\end{figure}

Esta falla ilustra un riesgo distinto al del alias arm\'onico simple discutido para Lomb--Scargle: cuando la l\'inea base observacional y el patr\'on de muestreo terrestre de un solo sitio (con sus brechas diarias y estacionales) permiten que alg\'un plegado a un periodo largo, no relacionado con la se\~nal f\'isica, produzca por azar una ocupaci\'on de fase m\'as concentrada que el plegado al periodo verdadero, el criterio de m\'inima entrop\'ia no tiene, por construcci\'on, ninguna forma de distinguir ese m\'inimo esp\'ureo de uno f\'isicamente significativo. Es precisamente esta clase de fallo -- ya se\~nalada por \citet{Graham2013} como una limitaci\'on persistente de los m\'etodos de orden de fase -- la que motiv\'o acotar el rango de b\'usqueda de las binarias a 0.1--100~d en lugar del rango completo utilizado con Lomb--Scargle (Secci\'on~\ref{sec:resultados-ls}): incluso con esta precauci\'on, las binarias siguen siendo el tipo de estrella m\'as dif\'icil para este m\'etodo, sin un patr\'on \'unico y dominante de error como el $P/2$ de Lomb--Scargle, sino una dispersi\'on amplia de razones $P_{\text{entrop\'ia}}/P_{\text{cat}}$ entre aproximadamente 0.25 y 20.

\subsection{Picos falsos por la cadencia de muestreo}
\label{sec:resultados-entropia-picos-falsos}

Adem\'as del m\'inimo global de inter\'es, los frecuenciogramas de entrop\'ia de pr\'acticamente todas las Cefeidas y RR Lyrae de la muestra exhiben un patr\'on adicional, visible incluso en las Figuras~\ref{fig:entropia-rrlyrae-ejemplo} y~\ref{fig:entropia-cefeida-falla}: numerosos m\'inimos locales estrechos y pronunciados, repartidos de forma m\'as o menos regular a lo largo del eje de frecuencia, que no guardan ninguna relaci\'on con el periodo real de la estrella ni con sus m\'ultiplos simples. Este comportamiento fue identificado y explicado en detalle por \citet{Henao2021} en un trabajo de grado dirigido, al igual que este, por el profesor Alejandro Garc\'ia, dedicado a comparar el periodograma de Lomb--Scargle con otros cuatro m\'etodos (incluido el de m\'inima entrop\'ia) sobre Cefeidas cl\'asicas de OGLE-IV. All\'i se muestra que, sin ninguna correcci\'on adicional, \emph{todos} los periodogramas estudiados presentan picos falsos cerca de la frecuencia correspondiente a la cadencia media de muestreo de los datos y sus m\'ultiplos, es decir, alrededor de 1~ciclo/d\'ia para fotometr\'ia terrestre de un solo sitio con cadencia aproximadamente nocturna como la de OGLE. La causa geom\'etrica, seg\'un \citet{Henao2021}, es que a esas frecuencias las observaciones, tomadas casi siempre a la misma hora local relativa a la fase, quedan agrupadas en una regi\'on angosta del diagrama de fase en lugar de distribuirse a lo largo de \'este; para el criterio de m\'inima entrop\'ia, cuya grilla fase--magnitud registra as\'i muchas celdas con probabilidad de ocupaci\'on nula, esto se traduce directamente en una entrop\'ia an\'omalamente baja, es decir, en un m\'inimo falso, sin que exista ninguna periodicidad f\'isica real en esa frecuencia.

\citet{Henao2021} encuentra adem\'as que, si bien este problema admite correcciones sencillas para los periodogramas de Fourier, Lomb--Scargle, dispersi\'on de fase y longitud de arco, \emph{no se hall\'o una soluci\'on confiable para el periodograma de m\'inima entrop\'ia}. Esto es plenamente consistente con la experiencia propia de este trabajo: como se mencion\'o en la Secci\'on~\ref{sec:metodologia-entropia}, \texttt{entropypf} enmascara expl\'icitamente, tras el c\'alculo de la entrop\'ia, los periodos de prueba cercanos a m\'ultiplos conocidos del d\'ia sid\'ereo (par\'ametro \texttt{aliases}), precisamente porque el problema no puede eliminarse en el c\'alculo mismo de la entrop\'ia. De hecho, para las binarias eclipsantes de este cap\'itulo fue necesario ampliar dicho enmascaramiento m\'as all\'a de lo que cubre \texttt{aliases=(1,)} por defecto: se verific\'o emp\'iricamente que la b\'usqueda de periodo converg\'ia, en algunos casos, a fracciones simples de un d\'ia no cubiertas por ese enmascaramiento est\'andar (por ejemplo, $P=0.75$~d $=3/4$~d), por lo que fue necesario enmascarar adicionalmente toda fracci\'on $m/n$ de un d\'ia con denominador peque\~no dentro del rango de b\'usqueda. Los periodos de m\'inima entrop\'ia reportados en la Tabla~\ref{tab:resultados-entropia} ya incorporan este enmascaramiento; las Figuras~\ref{fig:entropia-rrlyrae-ejemplo} y~\ref{fig:entropia-cefeida-falla}, en cambio, muestran deliberadamente la entrop\'ia bruta, sin enmascarar, precisamente para exhibir este problema en su totalidad.

\subsection{Aciertos acumulados por orden de alias en binarias}
\label{sec:resultados-binarias-alias}

Tanto la Tabla~\ref{tab:resultados-ls} como la Tabla~\ref{tab:resultados-entropia} reportan una \'unica tasa de aciertos con correcci\'on de alias, que solo considera candidatos hasta el m\'ultiplo $\times3$ del periodo ($P$, $P/2$, $2P$, $P/3$, $3P$). Dado que las binarias eclipsantes son, para ambos m\'etodos, el tipo de estrella m\'as afectado por esta degeneraci\'on arm\'onica, la Tabla~\ref{tab:binarias-aciertos-alias} desagrega, solo para ellas, la tasa de aciertos de forma acumulativa a medida que se aceptan \'ordenes de alias cada vez m\'as altos, hasta $\times6$: el mismo rango de m\'ultiplos ahora graficado en el panel derecho de las Figuras~\ref{fig:ls-resumen} y~\ref{fig:entropia-resumen}.

\begin{table}[H]
	\centering
	\begin{tabular}{ccc}
		\hline
		\textbf{M\'ultiplos de alias aceptados} & \textbf{Aciertos Lomb--Scargle (1\%)} & \textbf{Aciertos m\'inima entrop\'ia (1\%)} \\
		\hline
		$\times1$ & 3.8\% & 14.6\% \\
		$\times1$--$\times2$ & 74.8\% & 30.8\% \\
		$\times1$--$\times3$ & 78.0\% & 32.0\% \\
		$\times1$--$\times4$ & 82.4\% & 34.8\% \\
		$\times1$--$\times5$ & 82.8\% & 35.8\% \\
		$\times1$--$\times6$ & 84.2\% & 37.0\% \\
		\hline
	\end{tabular}
	\caption[Aciertos acumulados por orden de alias para binarias, Lomb--Scargle y m\'inima entrop\'ia]{Tasa de aciertos (error relativo del periodo menor al 1\%) para las 500 binarias eclipsantes de la muestra, acumulada a medida que se acepta como acierto cualquier candidato $P/n$ o $nP$ con $n$ hasta el orden indicado en cada fila. Cada fila incluye a las estrellas de las filas anteriores m\'as las que solo se recuperan correctamente en el nuevo orden de alias.}
	\label{tab:binarias-aciertos-alias}
\end{table}

El salto m\'as grande ocurre, para ambos m\'etodos, al incorporar el m\'ultiplo $\times2$: en Lomb--Scargle la tasa de aciertos pasa de 3.8\% a 74.8\%, consistente con el patr\'on dominante de alias en $P/2$ discutido en la Secci\'on~\ref{sec:resultados-ls}; en el m\'etodo de m\'inima entrop\'ia el salto es m\'as moderado, de 14.6\% a 30.8\%, reflejo de la dispersi\'on m\'as amplia de razones $P_{\text{entrop\'ia}}/P_{\text{cat}}$ se\~nalada en la Secci\'on~\ref{sec:resultados-entropia}. M\'as all\'a de $\times3$, cada m\'ultiplo adicional aporta mejoras marginales y decrecientes en ambos m\'etodos, lo que sugiere que extender la correcci\'on de alias m\'as all\'a de $\times3$ o $\times4$ tiene un costo creciente en falsos positivos por cada acierto adicional que efectivamente recupera.

\section{S\'intesis de la caracterizaci\'on individual}
\label{sec:sintesis-resultados}

Los resultados de este cap\'itulo son consistentes con el panorama planteado en la Secci\'on~\ref{sec:motivacion}, aunque con un matiz importante. Lomb--Scargle es extremadamente preciso y confiable para morfolog\'ias cercanas a una sinusoide (Cefeidas, RR Lyrae), pero falla de forma sistem\'atica y f\'acilmente caracterizable ante los eclipses de las binarias, bloque\'andose en el arm\'onico doble de la frecuencia orbital. El m\'etodo de m\'inima entrop\'ia, aplicado de forma individual y sin ajuste adicional de sus par\'ametros ($L$, $K$, remoci\'on de aliases) para cada morfolog\'ia, no solo hereda una versi\'on m\'as amplia de esa misma degeneraci\'on arm\'onica, sino que adem\'as resulta vulnerable a m\'inimos esp\'ureos de largo periodo bajo el muestreo irregular de un solo sitio terrestre, con un desempe\~no inferior al de Lomb--Scargle incluso en Cefeidas y RR Lyrae. Ninguno de los dos m\'etodos, por separado, resuelve consistentemente el problema de las binarias eclipsantes. Esta caracterizaci\'on independiente confirma, con datos reales y no solo con el argumento cualitativo de la Secci\'on~\ref{sec:motivacion}, la necesidad del esquema de integraci\'on propuesto en la Secci\'on~\ref{sec:metodo-combinado}: la capacidad de Lomb--Scargle para acotar eficientemente la regi\'on de b\'usqueda relevante, combinada con la sensibilidad morfol\'ogica del criterio de entrop\'ia una vez descartados los m\'inimos esp\'ureos de largo periodo, es lo que se espera que permita superar las limitaciones espec\'ificas de cada m\'etodo documentadas en este cap\'itulo.
